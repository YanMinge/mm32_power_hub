...
static void adc_filter(void)
{
    uint16_t channel;
    uint16_t cntFilter;
    uint32_t lADCFilterValue[ADC_SCANNUM] = {0, 0, 0};
...
int drv_gpio_pwm_mode(gpio_pin_type pin, uint16_t frequency)
{
    ...
    switch(pin)
    {
        ...
        case PB_7:       //TIM1_CH3
             /* GPIO configure */
             GPIO_PinAFConfig(GPIOB, GPIO_PinSource7, GPIO_AF_6);
             GPIO_InitStructure.GPIO_Pin  =  pin_num;
             GPIO_Init(GPIOB, &GPIO_InitStructure);
...
int drv_gpio_analog_write(gpio_pin_type pin, uint16_t analog_val)
{
    ...
    switch(pin)
    {
        ...
        case PB_3:       //TIM2_CH2
             ...
             TIM_SetCompare2(TIM2, capture_compare_val);
             break;
        case PB_4:       //TIM3_CH1
             capture_compare_val = line_map(analog_val, 0, pwm_divination, 0, 1000);
        ...
        case PB_5:       //TIM1_CH1
             capture_compare_val = line_map(analog_val, 0, pwm_divination, 0, 1000);
             TIM_SetCompare1(TIM1, capture_compare_val);
             break;
        case PB_6:       //TIM1_CH2
             capture_compare_val = line_map(analog_val, 0, pwm_divination, 0, 1000);
             TIM_SetCompare2(TIM1, capture_compare_val);
             break;
        case PB_7:       //TIM1_CH3
             capture_compare_val = line_map(analog_val, 0, pwm_divination, 0, 1000);
             TIM_SetCompare3(TIM1, capture_compare_val);
             break;
        case PB_13:
             break;
        case PB_14:
